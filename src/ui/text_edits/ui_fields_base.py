...
def config_to_fields(self):
    try:
        ...
        source_path = logs_and_config.read_config_value("Pfade", SOURCE)
        self.sett_paths_dlg.ui.source_path_text.setPlainText(source_path)
        DIRS.paths[SOURCE] = source_path

        template_1_path = logs_and_config.read_config_value("Pfade", TEMPLATE_1)
        self.sett_paths_dlg.ui.source_path_text_matstr.setPlainText(template_1_path)
        DIRS.paths[TEMPLATE_1] = template_1_path

        template_2_path = logs_and_config.read_config_value("Pfade", TEMPLATE_2)
        self.sett_paths_dlg.ui.source_path_text_docu.setPlainText(template_2_path)
        DIRS.paths[TEMPLATE_2] = template_2_path

        #!################ MainWindow ###########################
        target_1_path = logs_and_config.read_config_value("Pfade", TARGET_1)
        self.ui.target_path_text.setPlainText(target_1_path)
        DIRS.paths[TARGET_1] = target_1_path

        target_2_path = logs_and_config.read_config_value("Pfade", TARGET_2)
        self.ui.target_path_text_2.setPlainText(target_2_path)
        DIRS.paths[TARGET_2] = target_2_path
...
def char_validation(self):
    ...
    uppercase_string = project_to_uppercase(self, current_string, cur_cursor_pos)
...
def get_mapped_context(self):
    return {
        ...
        "{{module_type}}": (
            self.ui.module_type_text,
            aggr_dev_list(self.ui.PV_modules_list, 2),
        ),
        "{{mounting_type}}": (self.ui.mounting_type_text, "data_20"),
        "{{hybrid_inverter_bool}}": (self.ui.hybrid_inverter_bool_text, "data_21"),
        "{{inverter_type}}": (
            self.ui.inverter_type_text,
            aggr_dev_list(self.ui.PV_inverters_list, 2),
        ),
        "{{inverter_SN}}": (
            self.ui.inverter_SN_text,
            aggr_dev_list(self.ui.PV_inverters_list, 4),
        ),
        "{{inverter_power}}": (
            self.ui.inverter_power_text,
            aggr_dev_list(self.ui.PV_inverters_list, 5),
        ),
        "{{commiss_date}}": (self.ui.commiss_date_text, "data_25"),
        "{{bat_inverter_type}}": (
            self.ui.bat_inverter_type_text,
            aggr_dev_list(self.ui.BAT_inverters_list, 2),
        ),
        "{{bat_inverter_SN}}": (
            self.ui.bat_inverter_SN_text,
            aggr_dev_list(self.ui.BAT_inverters_list, 4),
        ),
        "{{bat_inverter_power}}": (
            self.ui.bat_inverter_power_text,
            aggr_dev_list(self.ui.BAT_inverters_list, 5),
        ),
        "{{coupling_type}}": (
            self.ui.coupling_type_text,
            aggr_dev_list(self.ui.BAT_inverters_list, 6),
        ),
        "{{bat_storage_type}}": (
            self.ui.bat_storage_type_text,
            aggr_dev_list(self.ui.BAT_storage_list, 2),
        ),
        "{{bat_storage_SN}}": (
            self.ui.bat_storage_SN_text,
            aggr_dev_list(self.ui.BAT_storage_list, 4),
        ),
        "{{bat_storage_cap}}": (
            self.ui.bat_storage_cap_text,
            aggr_dev_list(self.ui.BAT_storage_list, 5),
        ),
        "{{bat_commiss_date}}": (self.ui.bat_commiss_date_text, "data_33"),
        "{{max_discharge_pow}}": (
            self.ui.max_discharge_pow_text,
            aggr_dev_list(self.ui.BAT_storage_list, 6),
        ),
        ...
        "{{charging_point_type}}": (
            self.ui.charging_point_type_text,
            aggr_dev_list(self.ui.CHG_point_list, 2),
        ),
        "{{charging_point_SN}}": (
            self.ui.charging_point_SN_text,
            aggr_dev_list(self.ui.CHG_point_list, 4),
        ),
...
def aggr_dev_list(device_list, column_index):
    ...
    for row in range(device_list.rowCount()):
        if not device_list.item(row, column_index) == None:
            value = value + f"{device_list.item(row, column_index).text()} ({str(k)})\n"
            k += 1
    value = value.rstrip("\n")
    return value
...
def replace_fields_in_doc(self, doc_path, template_path):
    ...
    template = DIRS.paths[template_path]
    doc = DIRS.paths[doc_path]
            ...
            for row in table.getElementsByType(TableRow):
                ...
                for cell in row.getElementsByType(TableCell):
                    ...
                    for text_node in cell.getElementsByType(text.P):
                        ...
                        cell_format = text_node.getAttribute("stylename")
                        ...
                        for field_name, field_val in context.items():
                            ...
                            if field_name in cell_text:
                                ...
                                for field_value in field_values:
                                    ...
                                    new_text_node = text.P(text=str(field_value))
